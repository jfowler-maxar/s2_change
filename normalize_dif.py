...
class indicies():
    ...
    def set_chip(self, input_path, chip_size=256):
        # assuming s2, 10980x10980
        # input
        in_ds = gdal.Open(input_path)
        geoTransform = in_ds.GetGeoTransform()
        in_ds = None
        chip_x = int(chip_size)
        chip_y = chip_x
        minx = geoTransform[0]
        miny = geoTransform[3]
    ...
    def norm_dif(self, input_path, band_red, band_nir, out_path):
        ...
        chip_x, chip_y, minx, miny, step_x, step_y = self.set_chip(input_path, 512)
        count_x = 0
        count_y = 0
        ...
        for x in range(0, xsize, chip_x):
            ...
            for y in range(0, ysize, chip_y):
                ...
                b3 = in_ds.GetRasterBand(band_red).ReadAsArray(x, y, cols, rows).astype('float32')
                b4 = in_ds.GetRasterBand(band_nir).ReadAsArray(x, y, cols, rows).astype('float32')

                b3_nan = np.where(b3 == -9999, np.nan,b3)
                b4_nan = np.where(b4 == -9999, np.nan,b4)
    ...
    def msavi_calc(self, input_path, band_nir, band_red, out_path):
        ...
        chip_x, chip_y, minx, miny, step_x, step_y = self.set_chip(input_path, 256)
        count_x = 0
        count_y = 0
