        ...
        for x in range(0, xsize, block_xsize):
            ...
            for y in range(0, ysize, block_ysize):
                ...
                for j in range(1, num_bands + 1):
                    band = ds.GetRasterBand(j).ReadAsArray(x, y, cols, rows).astype(
                        'uint16')  # first load up of "current"
                    band_0 = ds_0.GetRasterBand(j).ReadAsArray(x, y, cols, rows).astype(
                        'uint16')  # load up previous date
